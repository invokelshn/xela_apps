...
class StdXelaTaxelViz2F : public rclcpp::Node {
public:
  StdXelaTaxelViz2F() : Node("std_xela_taxel_viz_2f") {
    ...
    sub_ = create_subscription<xela_taxel_msgs::msg::XTaxelSensorTArray>(
      in_topic_, 10, std::bind(&StdXelaTaxelViz2F::onArray, this, std::placeholders::_1));
    set_mode_service_ = create_service<std_srvs::srv::SetBool>(
      "std_xela_taxel_viz_2f/set_mode",
      std::bind(
        &StdXelaTaxelViz2F::onSetMode, this,
        std::placeholders::_1, std::placeholders::_2));

    RCLCPP_INFO(
      get_logger(), "std_xela_taxel_viz_2f started. in: %s out: %s service: ~/set_mode",
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(std::make_shared<StdXelaTaxelViz2F>());
  rclcpp::shutdown();
  return 0;
}
