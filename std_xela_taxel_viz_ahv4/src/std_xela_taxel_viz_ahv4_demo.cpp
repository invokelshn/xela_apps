...
class StdXelaTaxelVizAhv4Demo : public rclcpp::Node {
public:
  StdXelaTaxelVizAhv4Demo() : Node("std_xela_taxel_viz_ahv4_demo") {
    ...
    timer_ = create_wall_timer(
        ...
        std::bind(&StdXelaTaxelVizAhv4Demo::onTimer, this));

    RCLCPP_INFO(get_logger(), "std_xela_taxel_viz_ahv4_demo started. out: %s", out_topic_.c_str());
  }
...
int main(int argc, char **argv) {
  ...
  try {
    auto node = std::make_shared<StdXelaTaxelVizAhv4Demo>();
    rclcpp::spin(node);
