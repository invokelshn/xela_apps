...
class XelaTaxelVizAhv4 : public rclcpp::Node {
public:
  XelaTaxelVizAhv4() : Node("xela_taxel_viz_ahv4") {
    declare_parameter<std::string>("in_topic", "/x_taxel_ah");
    declare_parameter<std::string>("out_topic", "/x_taxel_ah/markers");
    declare_parameter<std::string>("frame_id", "x_taxel_ah_viz");
    ...
    sub_ = create_subscription<xela_taxel_msgs::msg::XTaxelSensorTArray>(
        in_topic_, qos,
        std::bind(&XelaTaxelVizAhv4::onArray, this, std::placeholders::_1));
    pub_ = create_publisher<visualization_msgs::msg::MarkerArray>(out_topic_, 10);

    RCLCPP_INFO(get_logger(), "xela_taxel_viz_ahv4 started. in: %s out: %s",
...
private:
  ...
  std::string jointToLinkName(const std::string &joint) const {
    ...
    if (joint.size() > suffix.size() &&
      ...
      return joint.substr(0, joint.size() - suffix.size()) + "_link";
    }
    return joint + "_link";
  }
...
int main(int argc, char **argv) {
  ...
  try {
    auto node = std::make_shared<XelaTaxelVizAhv4>();
    rclcpp::spin(node);
