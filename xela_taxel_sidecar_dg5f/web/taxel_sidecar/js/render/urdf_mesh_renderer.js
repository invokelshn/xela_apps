function parseFloatTriple(text, fallback) {
  if (!text) return fallback;
  const parts = String(text).trim().split(/\s+/).map((v) => Number(v));
  if (parts.length !== 3 || parts.some((v) => Number.isNaN(v))) {
    return fallback;
  }
  return parts;
}

function urdfRpyToQuaternion(roll, pitch, yaw) {
  const hr = roll * 0.5;
  const hp = pitch * 0.5;
  const hy = yaw * 0.5;
  const cr = Math.cos(hr);
  const sr = Math.sin(hr);
  const cp = Math.cos(hp);
  const sp = Math.sin(hp);
  const cy = Math.cos(hy);
  const sy = Math.sin(hy);
  return {
    qx: (sr * cp * cy) - (cr * sp * sy),
    qy: (cr * sp * cy) + (sr * cp * sy),
    qz: (cr * cp * sy) - (sr * sp * cy),
    qw: (cr * cp * cy) + (sr * sp * sy),
  };
}

function packageUriToHttpPath(uri) {
  const text = String(uri || "");
  if (!text.startsWith("package://")) {
    return text;
  }
  const rem = text.slice("package://".length);
  const slash = rem.indexOf("/");
  if (slash <= 0) {
    return "";
  }
  const pkg = rem.slice(0, slash);
  const rel = rem
    .slice(slash + 1)
    .replace(/^\/+/, "")
    .replace(/\/{2,}/g, "/");
  return `/pkg/${pkg}/${rel}`;
}

function parseRgbaToHex(rgbaText) {
  if (!rgbaText) return null;
  const parts = String(rgbaText).trim().split(/\s+/).map((v) => Number(v));
  if (parts.length < 3 || parts.slice(0, 3).some((v) => Number.isNaN(v))) {
    return null;
  }
  const toByte = (value) => {
    const clamped = Math.min(1.0, Math.max(0.0, value));
    return Math.round(clamped * 255);
  };
  return (toByte(parts[0]) << 16) | (toByte(parts[1]) << 8) | toByte(parts[2]);
}

function parseNamedMaterialColors(xmlDoc) {
  const out = new Map();
  for (const materialEl of Array.from(xmlDoc.getElementsByTagName("material"))) {
    const name = String(materialEl.getAttribute("name") || "").trim();
    if (!name) continue;
    const colorEl = materialEl.getElementsByTagName("color")[0];
    if (!colorEl) continue;
    const colorHex = parseRgbaToHex(colorEl.getAttribute("rgba"));
    if (colorHex === null) continue;
    out.set(name, colorHex);
  }
  return out;
}

function parseUrdfVisualSpec(xmlText) {
  const parser = new DOMParser();
  const xml = parser.parseFromString(xmlText, "application/xml");
  if (xml.getElementsByTagName("parsererror").length > 0) {
    return [];
  }
  const namedMaterialColors = parseNamedMaterialColors(xml);
  const linkSpecs = [];
  const linkElements = Array.from(xml.getElementsByTagName("link"));
  for (const linkEl of linkElements) {
    const linkName = linkEl.getAttribute("name");
    if (!linkName) continue;
    const visuals = [];
    let linkColorHex = null;
    const visualEls = Array.from(linkEl.getElementsByTagName("visual"));
    for (const visualEl of visualEls) {
      const geometryEl = visualEl.getElementsByTagName("geometry")[0];
      if (!geometryEl) continue;
      const meshEl = geometryEl.getElementsByTagName("mesh")[0];
      if (!meshEl) continue;
      const filename = meshEl.getAttribute("filename");
      if (!filename) continue;
      const ext = (filename.match(/\.([a-zA-Z0-9]+)(?:\?|#|$)/)?.[1] || "").toLowerCase();
      if (ext !== "stl" && ext !== "dae") continue;
      const originEl = visualEl.getElementsByTagName("origin")[0];
      const materialEl = visualEl.getElementsByTagName("material")[0];
      let visualColorHex = null;
      if (materialEl) {
        const localColorEl = materialEl.getElementsByTagName("color")[0];
        if (localColorEl) {
          visualColorHex = parseRgbaToHex(localColorEl.getAttribute("rgba"));
        }
        if (visualColorHex === null) {
          const materialName = String(materialEl.getAttribute("name") || "").trim();
          if (materialName && namedMaterialColors.has(materialName)) {
            visualColorHex = namedMaterialColors.get(materialName);
          }
        }
      }
      visuals.push({
        filename,
        ext,
        scale: parseFloatTriple(meshEl.getAttribute("scale"), [1, 1, 1]),
        xyz: parseFloatTriple(originEl ? originEl.getAttribute("xyz") : "", [0, 0, 0]),
        rpy: parseFloatTriple(originEl ? originEl.getAttribute("rpy") : "", [0, 0, 0]),
        urdfColorHex: visualColorHex,
      });
      if (linkColorHex === null && visualColorHex !== null) {
        linkColorHex = visualColorHex;
      }
    }
    if (visuals.length > 0) {
      linkSpecs.push({ linkName, visuals, urdfColorHex: linkColorHex });
    }
  }
  return linkSpecs;
}

export function createUrdfMeshRenderer({
  state,
  urdf3dLayer,
  normalizeFrameId,
  setHintText,
  safeErrorText,
  applyOrbitProfileForMode,
  getLinkMaterial,
  extractModuleIdFromName,
  onOrbitInteraction,
}) {
  const resolveModuleIdFromName =
    typeof extractModuleIdFromName === "function" ? extractModuleIdFromName : (() => null);

  function parseRootLink(xmlText) {
    const parser = new DOMParser();
    const xml = parser.parseFromString(xmlText, "application/xml");
    if (xml.getElementsByTagName("parsererror").length > 0) {
      return "";
    }
    const links = new Set();
    const childLinks = new Set();
    for (const linkEl of Array.from(xml.getElementsByTagName("link"))) {
      const name = normalizeFrameId(linkEl.getAttribute("name"));
      if (name) links.add(name);
    }
    for (const jointEl of Array.from(xml.getElementsByTagName("joint"))) {
      const childEl = jointEl.getElementsByTagName("child")[0];
      const childName = normalizeFrameId(childEl?.getAttribute("link"));
      if (childName) childLinks.add(childName);
    }
    for (const link of links) {
      if (!childLinks.has(link)) {
        return link;
      }
    }
    return links.size ? Array.from(links)[0] : "";
  }

  function resize() {
    const m = state.urdfMesh;
    if (!m.ready) return;
    const rect = urdf3dLayer.getBoundingClientRect();
    const w = Math.max(1, rect.width);
    const h = Math.max(1, rect.height);
    m.camera.aspect = w / h;
    m.camera.updateProjectionMatrix();
    m.renderer.setSize(w, h, false);
  }

  async function ensureRuntime() {
    const m = state.urdfMesh;
    if (m.ready) return true;
    if (m.loading) return false;
    if (m.failed) return false;

    m.loading = true;
    try {
      const [THREEMod, controlsMod, stlMod, colladaMod] = await Promise.all([
        import("../../vendor/three.module.js"),
        import("../../vendor/OrbitControls.js"),
        import("../../vendor/STLLoader.js"),
        import("../../vendor/ColladaLoader.js"),
      ]);
      const THREE = THREEMod;
      const { OrbitControls } = controlsMod;
      const { STLLoader } = stlMod;
      const { ColladaLoader } = colladaMod;

      const scene = new THREE.Scene();
      scene.background = new THREE.Color(0x0a1222);

      const camera = new THREE.PerspectiveCamera(48, 1, 0.001, 20.0);
      camera.up.set(0, 0, 1);
      camera.position.set(0.18, -0.22, 0.16);

      const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: false });
      // AH sets no toneMapping (leaves three.js's default NoToneMapping) --
      // matched here (2026-08-24) after finding this was the one remaining
      // difference between the two renderers' setup once lights/material were
      // aligned; ACESFilmicToneMapping was added on 2026-08-23 to fight a
      // washout that turned out to have other causes (arrow markers,
      // directional-light falloff) and was never present in AH's own recipe.
      renderer.setPixelRatio(Math.max(1, window.devicePixelRatio || 1));
      renderer.domElement.style.width = "100%";
      renderer.domElement.style.height = "100%";
      renderer.domElement.style.display = "block";
      urdf3dLayer.innerHTML = "";
      urdf3dLayer.appendChild(renderer.domElement);
      renderer.domElement.addEventListener("contextmenu", (ev) => ev.preventDefault());

      const controls = new OrbitControls(camera, renderer.domElement);
      controls.enableDamping = true;
      controls.mouseButtons = {
        LEFT: THREE.MOUSE.ROTATE,
        MIDDLE: THREE.MOUSE.DOLLY,
        RIGHT: THREE.MOUSE.PAN,
      };
      controls.target.set(0.0, 0.0, 0.0);
      controls.update();
      if (typeof onOrbitInteraction === "function") {
        const notifyOrbitInteraction = (kind) => () => onOrbitInteraction(kind);
        controls.addEventListener("start", notifyOrbitInteraction("start"));
        controls.addEventListener("end", notifyOrbitInteraction("end"));
        renderer.domElement.addEventListener("pointerdown", notifyOrbitInteraction("pointerdown"));
        renderer.domElement.addEventListener("wheel", notifyOrbitInteraction("wheel"), { passive: true });
      }

      // Real root cause of every "washes to white regardless of our lighting"
      // symptom (2026-08-24, confirmed via full scene.traverse() light scan --
      // a shallow scene.children check had missed it): several DG-5F visual
      // .dae files carry a leftover <library_lights> block from their
      // original CAD/DCC export, and ColladaLoader faithfully instantiates
      // those as real THREE.PointLight objects parented into the loaded mesh
      // -- see the embeddedLights strip in buildRobotFromDescription() below,
      // which removes them. Every earlier lighting experiment on this file
      // (removing directional lights, Lambert vs Standard material, tuning
      // ambient) was fighting those hidden point lights, not the setup here.
      // Now that they're actually gone, AH's exact recipe (ambient 0.75 + 2
      // directional) is restored -- it was never the problem.
      const amb = new THREE.AmbientLight(0xd0e3ff, 0.75);
      scene.add(amb);
      const dirA = new THREE.DirectionalLight(0xffffff, 0.9);
      dirA.position.set(0.7, -0.8, 1.2);
      scene.add(dirA);
      const dirB = new THREE.DirectionalLight(0x93c5fd, 0.4);
      dirB.position.set(-0.6, 0.8, 0.7);
      scene.add(dirB);

      // TEMP DIAGNOSTIC (2026-08-24): expose live objects for console inspection.
      window.__xelaDbg = { THREE, scene, renderer, camera };

      const rootGroup = new THREE.Group();
      const robotGroup = new THREE.Group();
      const markerGroup = new THREE.Group();
      const worldGrid = new THREE.GridHelper(1.4, 28, 0x3b4f73, 0x1f3256);
      worldGrid.rotation.x = Math.PI / 2;
      worldGrid.position.set(0, 0, 0);
      worldGrid.visible = false;
      rootGroup.add(worldGrid);
      rootGroup.add(robotGroup);
      rootGroup.add(markerGroup);
      scene.add(rootGroup);

      m.THREE = THREE;
      m.scene = scene;
      m.camera = camera;
      m.renderer = renderer;
      m.controls = controls;
      m.stlLoader = new STLLoader();
      m.colladaLoader = new ColladaLoader();
      m.rootGroup = rootGroup;
      m.robotGroup = robotGroup;
      m.markerGroup = markerGroup;
      m.worldGrid = worldGrid;
      m.ready = true;
      m.loading = false;
      applyOrbitProfileForMode();
      resize();
      return true;
    } catch (err) {
      m.failed = true;
      m.loading = false;
      m.failReason = err instanceof Error ? err.message : String(err);
      setHintText(`URDF mesh renderer unavailable: ${m.failReason}`);
      return false;
    }
  }

  async function getMeshAsset(url, ext) {
    const m = state.urdfMesh;
    const kind = (ext || "").toLowerCase();
    const key = `${kind}:${url}`;
    if (m.meshCache.has(key)) {
      return m.meshCache.get(key);
    }
    const promise = new Promise((resolve, reject) => {
      if (kind === "dae") {
        m.colladaLoader.load(
          url,
          (collada) => resolve({ kind: "dae", data: collada.scene }),
          undefined,
          (error) => reject(error),
        );
        return;
      }
      m.stlLoader.load(
        url,
        (geometry) => resolve({ kind: "stl", data: geometry }),
        undefined,
        (error) => reject(error),
      );
    });
    m.meshCache.set(key, promise);
    return promise;
  }

  async function buildRobotFromDescription() {
    const m = state.urdfMesh;
    if (!m.ready || !m.robotDescriptionXml) return false;

    const specs = parseUrdfVisualSpec(m.robotDescriptionXml);
    if (!specs.length) {
      m.failReason = "No mesh visuals parsed from robot_description.";
      m.robotBuilt = false;
      return false;
    }

    let meshLoadAttemptCount = 0;
    let meshLoadSuccessCount = 0;
    let meshLoadFailCount = 0;
    let firstMeshError = "";

    m.robotBuildToken += 1;
    const token = m.robotBuildToken;
    m.linkGroups.clear();
    m.linkModuleMap.clear();
    m.linkUrdfColorMap.clear();
    m.linkMeshMap.clear();
    m.robotGroup.clear();
    m.markerMap.clear();
    m.markerGroup.clear();

    const THREE = m.THREE;
    const pending = [];
    for (const linkSpec of specs) {
      const normalizedLinkName = normalizeFrameId(linkSpec.linkName) || linkSpec.linkName;
      m.linkModuleMap.set(normalizedLinkName, resolveModuleIdFromName(normalizedLinkName));
      if (Number.isInteger(linkSpec.urdfColorHex)) {
        m.linkUrdfColorMap.set(normalizedLinkName, linkSpec.urdfColorHex);
      }
      const linkMeshes = [];
      m.linkMeshMap.set(normalizedLinkName, linkMeshes);

      const linkGroup = new THREE.Group();
      linkGroup.name = linkSpec.linkName;
      linkGroup.visible = false;
      m.linkGroups.set(linkSpec.linkName, linkGroup);
      m.robotGroup.add(linkGroup);

      for (const visual of linkSpec.visuals) {
        const meshUrl = packageUriToHttpPath(visual.filename);
        if (!meshUrl) continue;
        meshLoadAttemptCount += 1;
        const task = getMeshAsset(meshUrl, visual.ext)
          .then((asset) => {
            if (token !== m.robotBuildToken) return;

            const visualRoot = new THREE.Group();
            visualRoot.position.set(visual.xyz[0], visual.xyz[1], visual.xyz[2]);
            const q = urdfRpyToQuaternion(visual.rpy[0], visual.rpy[1], visual.rpy[2]);
            visualRoot.quaternion.set(q.qx, q.qy, q.qz, q.qw);
            visualRoot.scale.set(visual.scale[0], visual.scale[1], visual.scale[2]);

            if (asset.kind === "dae") {
              const rootObj = asset.data.clone(true);
              if (
                Math.abs(rootObj.rotation.x + (Math.PI / 2)) < 1e-6 &&
                Math.abs(rootObj.rotation.y) < 1e-6 &&
                Math.abs(rootObj.rotation.z) < 1e-6
              ) {
                rootObj.rotation.set(0, 0, 0);
                rootObj.updateMatrixWorld(true);
              }
              // Root cause of the persistent, ambient-intensity-independent
              // "front of hand always washes to white" report (2026-08-24,
              // confirmed via a full scene.traverse() light scan that a naive
              // scene.children check missed): several DG-5F visual .dae files
              // (e.g. rl_dg_1_tip.dae, dg_sensor_type1.dae) carry a
              // <library_lights> block left over from the original CAD/DCC
              // export. ColladaLoader faithfully instantiates those as real
              // THREE.PointLight objects and parents them into the loaded
              // scene graph, so every one of those meshes silently added its
              // own bright (intensity 1) point light into our shared scene --
              // completely independent of whatever we set on our own
              // AmbientLight. Strip any embedded lights before the mesh is
              // added, same pass as the material override.
              const embeddedLights = [];
              rootObj.traverse((node) => {
                if (!node) return;
                if (node.isLight) {
                  embeddedLights.push(node);
                  return;
                }
                if (!node.isMesh) return;
                node.material = getLinkMaterial(THREE, linkSpec.linkName);
                linkMeshes.push(node);
              });
              for (const light of embeddedLights) {
                light.parent?.remove(light);
              }
              visualRoot.add(rootObj);
            } else {
              const material = getLinkMaterial(THREE, linkSpec.linkName);
              const mesh = new THREE.Mesh(asset.data, material);
              linkMeshes.push(mesh);
              visualRoot.add(mesh);
            }

            linkGroup.add(visualRoot);
            meshLoadSuccessCount += 1;
          })
          .catch((err) => {
            meshLoadFailCount += 1;
            if (!firstMeshError) {
              firstMeshError = `${meshUrl}: ${safeErrorText(err)}`;
            }
            console.warn("[xela-sidecar] mesh load failed:", meshUrl, err);
          });
        pending.push(task);
      }
    }

    await Promise.all(pending);
    if (token !== m.robotBuildToken) return false;

    if (meshLoadAttemptCount > 0 && meshLoadSuccessCount === 0) {
      m.robotBuilt = false;
      m.failReason =
        `No URDF meshes loaded (attempted=${meshLoadAttemptCount}, failed=${meshLoadFailCount}). ` +
        `${firstMeshError || "Check /pkg/<package>/... resource path."}`;
      return false;
    }

    if (meshLoadAttemptCount > 0 && meshLoadFailCount > 0) {
      console.warn(
        `[xela-sidecar] partial mesh load: success=${meshLoadSuccessCount}, failed=${meshLoadFailCount}`,
      );
    }
    m.failReason = "";
    m.robotBuilt = true;
    m.autoFrameDone = false;
    return true;
  }

  function refreshLinkMaterials(resolveLinkColor) {
    const m = state.urdfMesh;
    if (!m.ready || !m.robotBuilt || typeof resolveLinkColor !== "function") {
      return;
    }
    const THREE = m.THREE;
    for (const [linkName, meshes] of m.linkMeshMap.entries()) {
      if (!Array.isArray(meshes) || meshes.length === 0) {
        continue;
      }
      const colorHex = resolveLinkColor(linkName);
      if (!Number.isInteger(colorHex)) {
        continue;
      }
      const material = getLinkMaterial(THREE, linkName, colorHex);
      for (const mesh of meshes) {
        if (!mesh || !mesh.isMesh) continue;
        if (Array.isArray(mesh.material)) {
          mesh.material = mesh.material.map(() => material);
        } else {
          mesh.material = material;
        }
      }
    }
  }

  return {
    parseRootLink,
    ensureRuntime,
    resize,
    buildRobotFromDescription,
    refreshLinkMaterials,
  };
}
