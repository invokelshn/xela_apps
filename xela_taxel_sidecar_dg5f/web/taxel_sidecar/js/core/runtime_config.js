export function buildRuntimeConfig(windowRef) {
  ...
  // Dedicated rosbridge instance (rosbridge_websocket_taxel_viewer, see
  // xela_taxel_sidecar_cpp.launch.py) for xela_atag_taxel_viewer's own service calls only --
  // NOT used for topic subscriptions. The main connection above is kept busy with /tf and other
  // high-rate topics, which starves service-call futures on rosbridge_server's single
  // SingleThreadedExecutor (confirmed 2026-08-26, reproduces even on the pre-existing
  // Re-baseline button); a second rosbridge node with no heavy subscriptions of its own has its
  // own executor and isn't subject to that starvation.
  const explicitTaxelViewerWs = params.get("taxel_viewer_ws");
  const taxelViewerWsCandidates = explicitTaxelViewerWs ? [explicitTaxelViewerWs] : [
    `${wsProto}://${windowRef.location.hostname}:9091`,
  ];

  const topic = params.get("topic") || "/x_taxel_dg5f/web_state";
  const topicNs = (params.get("ns") || "/xvizdg5f").replace(/\/+$/, "");
  ...
  const vizNodeName = params.get("viz_node") || "/xvizdg5f/std_xela_taxel_viz_dg5f";
  ...
  // Priority: URL param → server-injected default (window.XELA_DG5F_SIM_SERVER_NODE) → ""
  // Uses a distinct window variable (XELA_DG5F_SIM_SERVER_NODE) to avoid conflict
  // with xela_taxel_sidecar_2f which uses XELA_SIM_SERVER_NODE.
  const simServerNode = (
    params.get("sim_server_node") ||
    (typeof windowRef !== "undefined" && windowRef.XELA_DG5F_SIM_SERVER_NODE) ||
    ""
  ).trim();

  return {
    wsCandidates,
    taxelViewerWsCandidates,
    topic,
    topicNs,
