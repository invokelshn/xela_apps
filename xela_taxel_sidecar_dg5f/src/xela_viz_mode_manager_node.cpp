...
public:
  ...
  : Node("xela_viz_mode_manager_cpp")
  {
    ...
    this->declare_parameter<std::string>("legacy_out_topic", "/x_taxel_dg5f/markers");
    ...
    this->declare_parameter<std::string>("managed_launch_package", "xela_taxel_sidecar_dg5f");
    ...
    this->declare_parameter<std::string>("viz_node_name", "/xvizdg5f/std_xela_taxel_viz_dg5f");
      ...
      [logger = this->get_logger(), mode](
        ...
        try {
          ...
          if (!response->success) {
            RCLCPP_WARN(
              logger,
              "std_xela_taxel_viz_dg5f rejected mode '%s': %s",
              mode.c_str(),
              response->message.c_str());
          }
        } catch (const std::exception & ex) {
          RCLCPP_WARN(logger, "std_xela_taxel_viz_dg5f mode request failed: %s", ex.what());
        }
      });
    return true;
  }
