...
// DG-5F is right-hand-only with 5 fingers + palm (AH's THUMB/INDEX/MIDDLE/RING/PALM
// 4-finger grouping does not apply). Module names come straight from
// r_server_model_joint_map.yaml, e.g. "f3_dg5f_ft", "f3_mid_uSPa22", "palm_uSPa46".
...
std::string extractModuleId(const std::string& joint_name) {
  // Format: x_taxel_<n>_{module_name}_{taxel_idx}_{joint|link}
  // module_name itself may contain underscores (e.g. "f3_mid_uSPa22"), unlike AH's
  // fixed single 2-digit numeric module token.
  ...
  if (last != "joint" && last != "link") {
    return "";
  }
  const std::string & taxel_idx = tokens[tokens.size() - 2];
  if (taxel_idx.empty() || taxel_idx.size() > 2 ||
    !std::all_of(taxel_idx.begin(), taxel_idx.end(), [](unsigned char c) {return std::isdigit(c);}))
  {
    return "";
  }
  // tokens[0]=x, tokens[1]=taxel, tokens[2]=<n>; module name spans tokens[3 .. size-3].
  std::string mod;
...
SensorGroup identifyGroup(const std::string& joint_name) {
  ...
  if (starts_with(mod, "f1_")) return SensorGroup::F1;
  if (starts_with(mod, "f2_")) return SensorGroup::F2;
  if (starts_with(mod, "f3_")) return SensorGroup::F3;
  if (starts_with(mod, "f4_")) return SensorGroup::F4;
  if (starts_with(mod, "f5_")) return SensorGroup::F5;
  if (starts_with(mod, "palm")) return SensorGroup::PALM;
...
class XelaTaxelWebBridgeDg5fNode : public rclcpp::Node {
public:
  XelaTaxelWebBridgeDg5fNode()
  : Node("xela_taxel_web_bridge_cpp")
  {
    ...
    sub_ = this->create_subscription<xela_taxel_msgs::msg::XTaxelSensorTArray>(
      in_topic_, 10,
      std::bind(&XelaTaxelWebBridgeDg5fNode::on_taxel_array, this, std::placeholders::_1));
    if (!full_in_topic_.empty() && full_in_topic_ != in_topic_) {
      full_sub_ = this->create_subscription<xela_taxel_msgs::msg::XTaxelSensorTArray>(
        full_in_topic_, 10,
        std::bind(&XelaTaxelWebBridgeDg5fNode::on_full_taxel_array, this, std::placeholders::_1));
    }
    if (include_slip_meta_) {
      slip_diagnostics_sub_ = this->create_subscription<std_msgs::msg::String>(
        slip_diagnostics_topic_,
        10,
        std::bind(&XelaTaxelWebBridgeDg5fNode::on_slip_diagnostics, this, std::placeholders::_1));
    }
    if (include_grasp_event_meta_) {
      grasp_event_sub_ = this->create_subscription<std_msgs::msg::String>(
        grasp_event_topic_,
        10,
        std::bind(&XelaTaxelWebBridgeDg5fNode::on_grasp_event, this, std::placeholders::_1));
    }
    on_set_parameters_handle_ = this->add_on_set_parameters_callback(
      std::bind(&XelaTaxelWebBridgeDg5fNode::on_set_parameters, this, std::placeholders::_1));

    if (use_shared_baseline_) {
      // transient_local so the CURRENT baseline (if xela_atag_taxel_viewer_node already computed
      // one) is delivered immediately on connect, not only on the next rebaseline -- matches that
      // node's own latched publisher QoS.
      shared_baseline_sub_ = this->create_subscription<std_msgs::msg::String>(
        shared_baseline_topic_,
        rclcpp::QoS(1).reliable().transient_local(),
        std::bind(&XelaTaxelWebBridgeDg5fNode::on_shared_baseline, this, std::placeholders::_1));
    }
    ...
    recalibrate_sub_ = this->create_subscription<std_msgs::msg::Empty>(
      ...
      [this](const std_msgs::msg::Empty::SharedPtr /*msg*/) {
        ...
        // Otherwise the next update_baseline() call reapplies this now-stale shared payload
        // (see apply_pending_shared_baseline()) and undoes the reset before self-baselining ever
        // gets a chance to run -- a fresh shared payload (e.g. from someone else's Rebaseline)
        // will repopulate this normally.
        pending_shared_baseline_.clear();
    ...
    reset_baseline_srv_ = this->create_service<std_srvs::srv::Trigger>(
        ...
        std_srvs::srv::Trigger::Response::SharedPtr resp)
      {
        ...
        std::fill(baseline_.force_sum.begin(), baseline_.force_sum.end(), Vec3{});
        std::fill(baseline_.taxel_sum.begin(), baseline_.taxel_sum.end(), Vec3{});
        pending_shared_baseline_.clear();  // see /xela_atag_recalibrate handler's comment above
...
private:
  ...
  {
    this->declare_parameter<std::string>("in_topic", "/x_taxel_dg5f");
    this->declare_parameter<std::string>("out_topic", "/x_taxel_dg5f/web_state");
    this->declare_parameter<std::string>("viz_mode", "grid");
    this->declare_parameter<std::string>("model_name", "XDG5FR");
    ...
    // DG-5F has no published sensor spec yet (new product); these default to
    // the AH uSCu (curved fingertip) sensor's own measured "H" sensitivity
    // range from xela_taxel_docs/uSCu_Specsheet_Oct_2024.pdf (Table 1:
    // x/y=+-1820 LSB, z=+18520 LSB) as a stand-in, per user direction
    // (2026-08-23) -- replace once DG-5F's own spec sheet is published.
    this->declare_parameter<double>("xy_taxel_range", 1820.0);
    this->declare_parameter<double>("z_taxel_range", 18520.0);
    ...
    // System-wide canonical baseline, computed once by xela_atag_taxel_viewer_node.py and
    // broadcast on a latched topic (2026-08-27 -- see that node's module docstring): this node's
    // OWN self-baseline above still runs as a startup/no-viewer-running fallback, but once a
    // baseline_state message arrives it overrides taxel_base/force_base directly so this node's
    // 3D view and the Session panel always agree on the same baseline, instead of each
    // self-baselining independently and drifting apart.
    this->declare_parameter<bool>("use_shared_baseline", true);
    this->declare_parameter<std::string>(
      "shared_baseline_topic", "/xela_atag_taxel_viewer_node/baseline_state");
    ...
    this->declare_parameter<bool>("show_f1", true);
    this->declare_parameter<bool>("show_f2", true);
    this->declare_parameter<bool>("show_f3", true);
    this->declare_parameter<bool>("show_f4", true);
    this->declare_parameter<bool>("show_f5", true);
    this->declare_parameter<bool>("show_palm", true);

    for (const char * mod : {
        "f1_dg5f_ft", "f2_dg5f_ft", "f3_dg5f_ft", "f4_dg5f_ft", "f5_dg5f_ft",
        "f1_mid_uSPa22", "f2_mid_uSPa22", "f3_mid_uSPa22", "f4_mid_uSPa22", "f5_mid_uSPa22",
        "f1_prox_uSPa22", "f2_prox_uSPa22", "f3_prox_uSPa22", "f4_prox_uSPa22", "f5_prox_uSPa22",
        "palm_uSPa46"})
    {
    ...
    this->declare_parameter<std::string>("full_in_topic", "/x_taxel_dg5f");
  ...
  {
    // DG-5F only ships a right-hand model/mapping today (no left-hand variant exists),
    // unlike AH which resolves left/right from hand_side_param_/model_name_. Kept as a
    // fixed function (rather than deleting hand_side entirely) for interface parity
    // with AH and to make a future left-hand DG-5F addition a small, contained change.
    return "right";
  }

  void load_parameters()
  {
    in_topic_ = get_string_param("in_topic", "/x_taxel_dg5f");
    out_topic_ = get_string_param("out_topic", "/x_taxel_dg5f/web_state");
    viz_mode_ = get_string_param("viz_mode", "grid");
    model_name_ = get_string_param("model_name", "XDG5FR");
    ...
    xy_taxel_range_ = std::max(1e-9, get_double_param("xy_taxel_range", 1820.0));
    z_taxel_range_ = std::max(1e-9, get_double_param("z_taxel_range", 18520.0));
    ...
    show_f1_ = get_bool_param("show_f1", true);
    show_f2_ = get_bool_param("show_f2", true);
    show_f3_ = get_bool_param("show_f3", true);
    show_f4_ = get_bool_param("show_f4", true);
    show_f5_ = get_bool_param("show_f5", true);
    ...
    slip_diagnostics_topic_ = get_string_param(
      "slip_diagnostics_topic", "/xela_atag_slip_detector/diagnostics");
    use_shared_baseline_ = get_bool_param("use_shared_baseline", true);
    shared_baseline_topic_ = get_string_param(
      "shared_baseline_topic", "/xela_atag_taxel_viewer_node/baseline_state");
    ...
    full_in_topic_ = get_string_param("full_in_topic", "/x_taxel_dg5f");
  ...
  void load_mapping()
  {
    ...
    // AH swaps the x_taxel_0_/x_taxel_1_ joint-name prefix depending on resolved hand
    // side, because its single mapping yaml is written for one side and reused for the
    // other. DG-5F's r_server_model_joint_map.yaml already uses the x_taxel_1_ prefix
    // that matches the live /x_taxel_dg5f message, and resolved_hand_side_ is always
    // "right", so this is a deliberate no-op kept for structural parity with AH.
    const std::string prefix_from = "x_taxel_1_";
    const std::string prefix_to = "x_taxel_1_";
    ...
    const std::chrono::steady_clock::time_point & now)
  {
    ...
    // ensure_baseline_size() resets ready/started whenever the taxel count changes (including
    // the very first call, 0 -> N) -- so a shared baseline applied BEFORE this node ever saw its
    // own first taxel message would otherwise get silently wiped out here. Cheap (~124 entries)
    // to just always reapply; no-ops once pending_shared_baseline_ is empty.
    apply_pending_shared_baseline();
  ...
  // Parses the system-wide baseline broadcast by xela_atag_taxel_viewer_node.py (2026-08-27)
  // into pending_shared_baseline_ -- application happens separately (apply_pending_shared_baseline,
  // called both here and from update_baseline()) because this can arrive over the wire BEFORE
  // this node has ever seen its own first taxel message, i.e. before baseline_.taxel_base/
  // force_base are even sized (see ensure_baseline_size()). Hand-rolled flat parse rather than a
  // JSON library dependency -- the payload's shape is fixed/internal (produced by that node
  // alone): {"stamp_sec":..,"duration_sec":..,"taxel_model_name":"..","baseline":{"frame_id":[z,x,y],...}}.
  // Keyed by frame_id, not (module_index, taxel_index), because that's the one identifier stable
  // across this node's own flat joint_to_index_ map and the publisher's own internal indexing.
  void on_shared_baseline(const std_msgs::msg::String::SharedPtr msg)
  {
    const std::string & s = msg->data;
    auto obj_pos = s.find("\"baseline\":{");
    if (obj_pos == std::string::npos) {
      return;
    }
    std::size_t pos = obj_pos + std::string("\"baseline\":{").size();
    pending_shared_baseline_.clear();
    shared_baseline_log_pending_ = true;
    while (pos < s.size() && s[pos] != '}') {
      if (s[pos] != '"') {
        ++pos;
        continue;
      }
      const auto key_end = s.find('"', pos + 1);
      if (key_end == std::string::npos) {
        break;
      }
      const std::string frame_id = s.substr(pos + 1, key_end - pos - 1);
      const auto bracket = s.find('[', key_end);
      const auto bracket_end = (bracket == std::string::npos) ? std::string::npos : s.find(']', bracket);
      if (bracket == std::string::npos || bracket_end == std::string::npos) {
        break;
      }
      const std::string nums = s.substr(bracket + 1, bracket_end - bracket - 1);
      double z = 0.0, x = 0.0, y = 0.0;
      if (std::sscanf(nums.c_str(), "%lf,%lf,%lf", &z, &x, &y) == 3) {
        pending_shared_baseline_[frame_id] = {x, y, z};
      }
      pos = bracket_end + 1;
    }
    apply_pending_shared_baseline();
  ...
  void apply_pending_shared_baseline()
  {
    if (pending_shared_baseline_.empty() || baseline_.taxel_base.empty()) {
      return;
    }
    std::size_t applied = 0;
    for (const auto & [frame_id, xyz] : pending_shared_baseline_) {
      const auto idx_opt = lookup_index_for_frame(frame_id);
      if (!idx_opt) {
        continue;
      }
      const auto idx = *idx_opt;
      if (idx < baseline_.taxel_base.size()) {
        baseline_.taxel_base[idx] = Vec3{xyz[0], xyz[1], xyz[2]};
      }
      if (idx < baseline_.force_base.size()) {
        baseline_.force_base[idx] = Vec3{xyz[0], xyz[1], xyz[2]};
      }
      ++applied;
    }
    if (applied > 0) {
      baseline_.ready = true;
      baseline_.started = true;
      if (shared_baseline_log_pending_) {
        // apply_pending_shared_baseline() is called from update_baseline() on EVERY message
        // (idempotent -- cheap to reapply the same ~124 entries, see call site comment), so only
        // log the first time a given payload gets successfully applied, not every frame.
        shared_baseline_log_pending_ = false;
        RCLCPP_INFO(
          this->get_logger(),
          "Applied shared baseline_state from %s (%zu taxels).",
          shared_baseline_topic_.c_str(), applied);
      }
    }
  }
  ...
  bool isIndexVisible(size_t idx) const {
    ...
    switch (g) {
      case SensorGroup::F1:   return show_f1_;
      case SensorGroup::F2:   return show_f2_;
      case SensorGroup::F3:   return show_f3_;
      case SensorGroup::F4:   return show_f4_;
      case SensorGroup::F5:   return show_f5_;
    ...
    const std::vector<rclcpp::Parameter> & params)
  {
    ...
    for (const auto & param : params) {
      ...
      } else if (name == "show_f1") {
        bool value = show_f1_;
        if (!parameter_to_bool(param, value)) {
          result.successful = false;
          result.reason = "show_f1 must be bool-compatible";
          return result;
        }
        show_f1_ = value;
      } else if (name == "show_f2") {
        bool value = show_f2_;
        if (!parameter_to_bool(param, value)) {
          result.successful = false;
          result.reason = "show_f2 must be bool-compatible";
          return result;
        }
        show_f2_ = value;
      } else if (name == "show_f3") {
        bool value = show_f3_;
        if (!parameter_to_bool(param, value)) {
          result.successful = false;
          result.reason = "show_f3 must be bool-compatible";
          return result;
        }
        show_f3_ = value;
      } else if (name == "show_f4") {
        bool value = show_f4_;
        if (!parameter_to_bool(param, value)) {
          result.successful = false;
          result.reason = "show_f4 must be bool-compatible";
          return result;
        }
        show_f4_ = value;
      } else if (name == "show_f5") {
        bool value = show_f5_;
        if (!parameter_to_bool(param, value)) {
          result.successful = false;
          result.reason = "show_f5 must be bool-compatible";
          return result;
        }
        show_f5_ = value;
  ...
  bool use_shared_baseline_{true};
  std::string shared_baseline_topic_{"/xela_atag_taxel_viewer_node/baseline_state"};
  std::unordered_map<std::string, std::array<double, 3>> pending_shared_baseline_;  // frame_id -> [x,y,z]
  bool shared_baseline_log_pending_{false};
  ...
  std::string full_in_topic_{"/x_taxel_dg5f"};
  ...
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr shared_baseline_sub_;
...
int main(int argc, char ** argv)
{
  ...
  try {
    auto node = std::make_shared<XelaTaxelWebBridgeDg5fNode>();
    rclcpp::spin(node);
