...
#include "xela_server2_dg5f/dg5f_parser.hpp"
...
class XelaServer2Dg5fNode : public rclcpp::Node {
 public:
  XelaServer2Dg5fNode() : Node("xela_server2_dg5f"), running_(true) {
    ws_host_ = declare_parameter<std::string>("ws_host", "localhost");
    ws_port_ = declare_parameter<int>("ws_port", 5000);
    hand_side_ = declare_parameter<std::string>("hand_side", "right");
    ...
    try {
      default_frame_ids = ament_index_cpp::get_package_share_directory("xela_server2_dg5f");
      default_frame_ids += "/config/" + map_file;
    } catch (const std::exception &e) {
      RCLCPP_WARN(get_logger(), "Failed to locate xela_server2_dg5f share: %s", e.what());
    ...
    timer_ = create_wall_timer(std::chrono::milliseconds(10),
                               std::bind(&XelaServer2Dg5fNode::OnTimer, this));

    if (input_json_path_.empty()) {
      ws_thread_ = std::thread(&XelaServer2Dg5fNode::WebsocketLoop, this);
      ...
      if (!ReadFileToString(input_json_path_, file_content, file_error)) {
        ...
        playback_timer_ = create_wall_timer(
            ...
            std::bind(&XelaServer2Dg5fNode::OnPlaybackTimer, this));
...
int main(int argc, char **argv) {
  ...
  auto node = std::make_shared<xela_server2_dg5f::XelaServer2Dg5fNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
